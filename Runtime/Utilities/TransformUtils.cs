...
namespace BioluminescentGames.Utils.Utilities
{
    public static class TransformUtils
    {
        ...
        {
            transform.ClearChildren(_ => true);
        }

        public static void ClearChildren(this Transform transform, Func<Transform, bool> predicate)
        {
            Queue<Transform> transformsToDestroy = new();
            foreach (Transform child in transform)
                if (predicate(child))
                    transformsToDestroy.Enqueue(child);

            while (transformsToDestroy.Count > 0)
            {
                Transform childToDestroy = transformsToDestroy.Dequeue();
                childToDestroy.gameObject.Destroy();
            }
        }
